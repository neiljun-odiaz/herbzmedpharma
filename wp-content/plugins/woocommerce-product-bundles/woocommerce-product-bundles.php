		...
		function woo_bundles_wishlist_after_list_item_name( $item, $wishlist ) {
			...
			if ( $item[ 'data' ]->is_type( 'bundle' ) && ! empty( $item[ 'stamp' ] ) ) {
				...
				foreach ( $item[ 'stamp' ] as $bundled_item_id => $bundled_item_data ) {
					...
					if ( ! empty ( $bundled_item_data[ 'attributes' ] ) && $woocommerce_bundle_helpers->is_wc_21() ) {
						$attributes = '';
				...
				echo '</dl>';
				echo '<p class="bundled_notice wishlist_component_notice">' . __( '*', 'woo-bundles' ) . '&nbsp;&nbsp;<em>' . __( 'Accurate pricing info available in cart.', 'woo-bundles' ) . '</em></p>';
			}
		}
